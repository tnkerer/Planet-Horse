// src/components/game/Stables/phaser/core/balanceService.ts
...
export class BalanceLiveStore {
  ...
  private notify() {
    for (const l of this.listeners) {
      try { l(this.snap); } catch { /* never throw to callers */ }
    }
  }
  ...
  private async fetchBalance(signal: AbortSignal): Promise<{
    changed: boolean;
    data?: { nickname: string | null; phorse: number; wron: number; medals: number; shards: number };
  }> {
    ...
    // Expected server payload now includes `shards`
    // { nickname, phorse, wron, medals, shards }
